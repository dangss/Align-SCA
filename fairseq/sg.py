...
class SequenceGenerator(object):
    ...
    def generate(
        ...
        **kwargs
        ...
        if 'src_tokens_lm' in encoder_input:
            src_tokens_lm = encoder_input['src_tokens_lm']
            lm_out = model.src_lm(src_tokens_lm)
            encoder_input['src_tokens_lm'] = lm_out
...
class EnsembleModel(torch.nn.Module):
    ...
    def _decode_one(
        ...
        if self.incremental_states is not None:
            decoder_out = list(model.decoder(tokens,None, encoder_out, incremental_state=self.incremental_states[model]))
        else:
